...
function applyEnvOverrides(config) {
  ...
  const nestedEnvMap = {
    ...
    CLAUDE_OUTPUT_FORMAT: ['claude', 'outputFormat'],
...
function validateConfig(config) {
  ...
  if (!validFormats.includes(config.claude.outputFormat)) {
    throw new Error(`claude.outputFormat must be 'text' or 'stream-json'`);
  }
