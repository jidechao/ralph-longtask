...
export async function buildPrompt(story, promptsConfig, prd) {
  ...
  const oversized = charCount > PROMPT_CHAR_LIMIT;

  return { prompt, charCount, sourceCount, oversized };
}

export function ensurePromptWithinLimit(promptResult, story) {
  if (!promptResult?.oversized) {
    return;
  }

  const storyId = story?.id || 'unknown-story';
  throw new Error(
    `Prompt too large for ${storyId}: ${promptResult.charCount} chars exceeds ${PROMPT_CHAR_LIMIT}. Split the story or trim context.`,
  );
}
